...
namespace NecromancersRising.UI
{
    public class MonsterHPBar : MonoBehaviour
    {
        ...
        {
            if (_targetMonster == null || _hpBarContainer == null || _mainCamera == null)
            {
                Debug.LogWarning($"MonsterHPBar missing references: Monster={_targetMonster}, Container={_hpBarContainer}, Camera={_mainCamera}");
                return;
            }
            ...
            {
                Debug.Log($"Monster: {_targetMonster.name}, WorldPos: {worldPos}, ScreenPos: {screenPos}, Screen.height: {Screen.height}");
            }
            ...
            if (screenPos.z > 0)
            {
                ...
                // Center the 120px wide bar by offsetting by half its width
                float leftPos = screenPos.x - 60;
