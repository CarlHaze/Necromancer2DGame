...
/// Uses Monte-Carlo simulation rules
/// </summary>
public class UndeadBattleTester : MonoBehaviour
{
    ...
    private void RunMatchup(UndeadData fighter1Data, UndeadData fighter2Data)
    {
        ...
        Debug.Log($"Note: Basic attacks deal neutral damage. Type effectiveness will apply to moves later.\n");
    }

    /// <summary>
    /// Simulates a single battle between two undead
    /// Uses Monte-Carlo simulation rules:
    /// - Turn order by speed; ties resolved randomly
    /// - No crits, abilities, items, status effects
    /// - Battle ends when one HP <= 0
    /// - Simultaneous KOs handled as random tie winner
    ...
    private string SimulateBattle(UndeadStats fighter1, UndeadStats fighter2)
    {
        ...
        while (fighter1.IsAlive && fighter2.IsAlive)
        {
            ...
            int damage1 = CalculateDamage(firstAttacker, secondAttacker);
            ...
            int damage2 = CalculateDamage(secondAttacker, firstAttacker);
